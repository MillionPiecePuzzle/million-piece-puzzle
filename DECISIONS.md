...
Choice: the four corner-anchored HUD panels (`ReferencePanel`, `LeaderboardPanel`, `ActivityTicker`, `MiniMap`) get `width: min(<natural>px, calc(50vw - 24px))`, so the two panels sharing a corner-row can never overlap at any viewport width. `TopBar`'s grid switches its flanking `1fr` columns to `minmax(0, 1fr)` with `min-width: 0` and text-ellipsis on the wordmark and pseudo, plus two breakpoints: 560px drops the playtime caption, 420px drops the wordmark and the presence flag/pseudo down to an icon-only settings button.
Why: neither had any responsive rule; both provably broke on phone-width viewports (panels overlap under ~600px; the topbar overflowed by 100+px under ~530px from CSS Grid's implicit `1fr` `min-width: auto`). Shrinking in place was chosen over hiding a panel outright, since none of the four currently has an alternate mobile entry point (no nav/drawer) to reach its content if hidden.
Revisit when: a genuine mobile nav (tab bar / drawer) is ever added, panels should collapse into that instead of shrinking to ~140-160px on a 375px phone. Verified via an isolated static harness reproducing the exact CSS (real `/play` needs a live WS session, no local backend was stood up for this); worth a real-device pass once one is next in hand.
