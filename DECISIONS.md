...
Revisit when: never expected for the versioning scheme. A board reset stays an unclosed gap: it clears the version index back to empty (`state.clearCellCompositeVersions`) rather than deleting the R2 objects, so a life that reaches fewer versions than the one before a reset leaves that tail permanently orphaned; accepted since resets are a rare admin action and R2 storage is cheap.
