...
### 2026-07-26, backend-realtime, Stage 4 ships with no wire or protocol change

Choice: Stage 4 builds, cascades, and backfills the pyramid entirely server-side; `region_state`'s `cellComposites` and the `cell_composite` push keep sending exactly what they send today (level 0 only), with no new field and no `PROTOCOL_VERSION` bump. Levels 1-3 exist and stay correct in R2 and the in-process index (verifiable directly, and by the backfill script) but reach no client until Stage 5.
Why: the wire's `cellKey` is packed identically at every level (a level-0 cell and a level-1 cell can share the same numeric key), and the deployed frontend's `applyCellComposite` takes no `level` argument at all; sending a level 1-3 entry now would make it unpack to the wrong world-space rect and get misapplied as if it were an oddly-sized level-0 tile, corrupting Stage 3's already-shipped rendering instead of extending it. Keeping the wire untouched makes Stage 4 safe to deploy fully independently of Stage 5, matching every other stage's own bar for independent shippability.
Revisit when: superseded by Stage 5, which is specifically the wire-exposure work this defers.
