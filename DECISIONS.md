...
- [Bounded per-cell piece-tile fetch concurrency and bake retries](#2026-08-03-backend-realtime-bounded-per-cell-piece-tile-fetch-concurrency-and-bake-retries) -> not yet re-verified by an actual admin resweep against the real ~99.5%-locked synthetic-1m prod board; `CELL_BAKE_MAX_ATTEMPTS` (3) is not yet tuned against a real persistent (non-transient) failure.
...
Revisit when: not yet re-verified by an actual admin resweep against the real ~99.5%-locked synthetic-1m prod board this was written for; `CELL_BAKE_MAX_ATTEMPTS` (3) is not yet tuned against a real persistent (non-transient) failure, as opposed to the transient CDN overload this was written to fix. If a cell still fails after all attempts, the give-up log line names it so it can be resweept by hand.
...
Revisit when: never expected; if straggler audits become routine rather than a rare post-incident cleanup, consider exposing this from the admin page's own UI instead of curl-only.
