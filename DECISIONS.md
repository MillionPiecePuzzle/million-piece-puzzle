...
Revisit when: `DZI_QUALITY` (85, decoupled from AVIF's `--quality`) is an eyeballed AVIF-60-equivalent estimate, not a measured match; retune by eye once the real photo replaces the synthetic one. The server still bakes and stores the full-photo composite tier alongside the mask/seam even though no client fetches it anymore; retiring that server-side bake and its R2 footprint is a separate, unstarted follow-up.
