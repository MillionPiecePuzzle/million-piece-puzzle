...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> now depends on R2 write credentials like the photo composite; full verification at a real R2-backed deployment and the replace-vs-coexist call with `CompositeTileLayer` are both still pending.
...
Choice: the DZI-native reveal (`?dziReveal=1`, off by default; `CompositeTileLayer` untouched and still the production renderer when the flag is off) no longer masks to locked pieces' rectangular bounds. `CellCompositor.processCell` now rasterizes a silhouette mask and a seam (border-only) texture alongside the unchanged photo composite, straight from `generatePieceGeometry` and the shared `piecePathD`/`pieceMaskSvg`/`pieceBorderSvg` helpers (now shared with the offline slicer rather than duplicated) off the seed already held in-process: no piece-tile fetch needed, unlike the photo composite. All three variants share one version bump and R2 key prefix (`<version>.avif` / `-mask.avif` / `-seam.avif`), so the client derives all three URLs from the same `{cellKey, version}` pair `region_state`/`cell_composite` already carry; no `PROTOCOL_VERSION` bump needed. Client-side, `DziRevealLayer` bakes every hydrated per-cell mask sprite into one combined `RenderTexture` (mirroring `LodTileLayer`'s own bake-to-texture technique) and assigns that single Sprite as `tileContainer.mask`, rather than a `Container` holding several sprites: confirmed directly (a WebGL pixel-readback harness, not just reading Pixi's source) that only a lone Sprite gets Pixi's real per-pixel `AlphaMask`, while a `Container` of sprites silently falls back to `StencilMask` (masks by each sprite's rectangular quad, reproducing the exact flat-rectangle bug this bake exists to fix). The seam texture draws directly on top, unmasked: the server only ever strokes a border where a piece is actually locked, so it never paints outside a valid reveal region on its own.
Why: reuses a pipeline and R2/versioning model already proven in prod instead of building a second one, and needs no wire change. Chosen over the other option considered, raw per-piece Bezier parameters sent to the client for locked pieces only: that path would hand a still-unlocked neighbor's exact required tab/socket shape in numeric form, a materially bigger anti-programmatic-solving exposure than a raster image a bot would still have to run CV against (already out of scope, see anti-programmatic-solving).
Revisit when: this bake now depends on R2 write credentials the same way the photo composite does (confirmed with the user before implementing), widening the existing local-dev gap ("no compositor configured means locked pieces do not render") to this path too. Full verification (`seed-lock-scenario` against a real R2-backed deployment, then a real-browser visual pass) is still pending, and whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided until then.
