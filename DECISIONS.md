...
- [Locked pieces stop being a group](#2026-07-23-backend-realtime-locked-pieces-stop-being-a-group) -> Stage 2 (flat per-cell locked delivery) not shipped yet; until then a restart or a fresh pan shows already-locked regions as empty rather than loading.
...
Revisit when: `GroupIndex`/`region_state` still key off Redis groups, so a locked piece has no index entry: after this alone (Stage 1 of the rollout), a server restart or a client panning into an unvisited cell shows already-locked regions as empty rather than loading, until Stage 2 ships the flat per-cell locked delivery (tracked in ROADMAP Phase 5, planned as a separate follow-up session).
