...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> a fifth real-scale pass found the antialias fix wasn't the whole story: `lodTiles.ts`'s zoom-out LOD bake shared the same anti-pattern (fixed, kept, but not the actual cause here), and the true cause was the main canvas's own antialias inherited into Pixi's mask filter pass (fixed by dropping it there); neither re-verified live, and whether the resulting loss of edge smoothing on small UI accents reads as acceptable is a genuine open question. The DZI pyramid re-slice/re-upload the quality fix needs still hasn't run, and the replace-vs-coexist call with `CompositeTileLayer` is still open.
