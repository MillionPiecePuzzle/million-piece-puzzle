...
Revisit when: the DZI pyramid quality fix still needs a real re-slice (`npm run slice`) and R2 re-upload (multi-GB) before it takes effect on `synthetic-1m`, not done yet; a sixth real-browser pass is needed to confirm every fix so far holds (photo sharpness at parity with `CompositeTileLayer`, no WebGL errors and full coverage through the whole zoom range including past 350%, and the `lodTiles.ts` fix above holding under ordinary zoom-out/pan) and that neither antialias removal reintroduced visible edge aliasing worth trading back. `DZI_QUALITY` (85) is an eyeballed estimate, not a measured AVIF-60 match; retune by eye once the real photo replaces the synthetic one, the same as the mosaic builder's own tile-quality entry. Whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided.
