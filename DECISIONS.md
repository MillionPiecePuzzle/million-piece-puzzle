...
- [Retire the server-side full-photo composite tier](#2026-08-05-backend-realtime-retire-the-server-side-full-photo-composite-tier) -> the already-orphaned photo objects on R2 still need a one-off bulk delete.
...
Revisit when: the photo objects already sitting on R2 from before this change (confirmed: 2560 bare `<version>.avif` objects under `synthetic-1m/cells/`, ~794 MiB, isolated from live mask/seam objects by the absence of any `-mask`/`-seam` suffix) still need a one-off bulk delete to reclaim; not yet done as of this entry.
