...
- [Per-cell compositing on a debounced dirty-cell queue](#2026-07-24-backend-realtime-per-cell-compositing-runs-on-a-debounced-dirty-cell-queue) -> the drain loop's one-at-a-time throughput is unmeasured against real bake volume.
- [Bounded per-cell piece-tile fetch concurrency and bake retries](#2026-08-03-backend-realtime-bounded-per-cell-piece-tile-fetch-concurrency-and-bake-retries) -> fully verified clean (1296/1296) against the real ~99.5%-locked synthetic-1m prod board, including the 2 cells blocked on a diagnosed R2/Cloudflare per-object serving fault (not a data or code bug); that failure class was contained to exactly those 2 cells with no cascade, but is not provably excluded from recurring on other objects during the real event, so watch for it there.
- [CompositeTileLayer is the only rendering path for locked content](#2026-07-26-frontend-canvas-compositetilelayer-is-the-only-rendering-path-for-locked-content) -> `COMPOSITE_VRAM_BUDGET_MB` is an unmeasured default against real per-tile byte cost.
- [seed-lock-scenario locks pieces through the real grab/drop path](#2026-07-27-qa-and-load-seed-lock-scenario-locks-pieces-through-the-real-grabdrop-path) -> verified at 10 000 and 100 000-piece scale (clean composite, `validate-state` passes, client rendering confirmed by eye); still not run at the real ~995 000-piece scale, and the compositor drain's throughput under sustained load needs more headroom than a linear scale-up suggests.
- [Composite pyramid collapsed back to a single level](#2026-07-27-qa-and-load-composite-pyramid-collapsed-back-to-a-single-level) -> the 100k-piece run confirms the full cell grid saturates well before full board lock; the client-side whole-board fetch/VRAM load at minimum zoom is still unverified, watch for it at the ~995 000-piece scale re-run.
...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> the sixth pass's `boundsArea` fix (clamped to `keepRing`, meant to close a non-antialias WebGL crash at 500% zoom) shipped a regression: `boundsArea` on a container that also carries a mask effect is a genuine Pixi gotcha, silently corrupting that container's own computed bounds. The seventh pass moved `boundsArea` to the effect-free `baseTileContainer` instead, but could not get a reliable synthetic reproduction either way (Pixi's bounds caching needs a real per-frame render pass a scripted test doesn't provide), so this is unverified against a real crash-to-no-crash transition, only against the specific regression mechanism. None of the fixes across all seven passes are confirmed live, and whether the antialias loss on small UI accents reads as acceptable is still a genuine open question. The DZI pyramid re-slice/re-upload the quality fix needs still hasn't run, and the replace-vs-coexist call with `CompositeTileLayer` is still open.
- [Mosaic builder inherits the 72px/piece gigapixel ceiling](#2026-07-29-image-pipeline-mosaic-builder-inherits-the-72pxpiece-gigapixel-ceiling) -> re-test after a libvips/sharp upgrade or on Linux; lifting the ceiling should move both scripts' defaults together.
- [seed-lock-scenario defers compositing to one end-of-run pass](#2026-07-31-qa-and-load-seed-lock-scenario-defers-compositing-to-one-end-of-run-pass) -> not yet re-verified at the real ~995 000-piece scale.
...
Choice: a new `CompositeTileLayer` (sibling to `LodTileLayer`, not merged into it: baking a live Pixi scene is synchronous, fetching a server AVIF is async with retry/timeout, different enough lifecycles to keep apart) fetches and shows every composited cell covering the current viewport as a `Sprite`, added directly to the existing `lockedPiecesLayer` (no new container needed: it already sits below `unlockedLayer` and the loose-piece LOD tile, which stays transparent wherever locked content lives since `bakeTile` no longer paints locked content into it at all). Sprites use the exact margin-widened bounds Stage 3 already validated. `hydrateLockedPiece`/`lockedHydrateQueue`/`lockedResident` and the whole per-piece fallback they existed for are removed rather than kept as a second path: a locked piece never gets its own fetch again, at any zoom.
Why: CompositeTileLayer's own byte-weighted budget (real decoded size, not a fixed "1 unit") replaces the pre-Stage-4 approximation that was safe only while composites were an opportunistic dedup on top of per-piece rendering, not the only path.
Revisit when: `COMPOSITE_VRAM_BUDGET_MB` (256, mirroring `LOD_VRAM_BUDGET_MB`) is an unmeasured default against real per-tile byte cost; tune once there is live data. See also collapsing the composite pyramid back to a single level below: at full board lock and minimum zoom, this layer can now be asked to hold every cell in the grid at once.
...
Revisit when: verified on prod's `synthetic-1m` at 10 000-piece scale (2026-07-27, before the composite pyramid was collapsed back to a single level): 10 051/10 051 pieces locked, all 1 274/1 274 touched cells composited, no gaps. Re-verified at 100 000-piece scale (2026-07-29, against the collapsed single-level compositor plus the DZI-reveal mask/seam bake): 99 614/99 614 pieces locked, `validate-state` passes every invariant, and the full 1 296/1 296-cell grid composited (already saturated at 10% piece density) once 5 cells an unrelated ~3h SSH disconnect had cut off mid-drain were identified by diffing `allCellKeysForGrid` against the Redis version index and manually re-driven. Client-side rendering confirmed visually in a real browser (the automated-preview limitation noted in the DZI-reveal entry below still blocks a scripted pass). Still not run at the real ~995 000-piece scale: this run's compositor drain notably slowed under sustained load as dirty cells piled up (roughly 38 -> 11 -> 8.5 pieces/s), compounded by the mask/seam bake now tripling each cell's upload work, so a 995k-scale run should budget meaningfully more wall-clock time than a linear scale-up from the 10k baseline would suggest, not just more pieces.

### 2026-07-27, qa-and-load, composite pyramid collapsed back to a single level

Choice: the multi-level composite pyramid (levels 0-3, ROADMAP Phase 5's former Stage 4) is removed; the server composites and serves only the one native-resolution tile per cell Stage 3 originally built (1296 cells), and the frontend's `CompositeTileLayer` shows exactly those tiles at every zoom instead of picking a coarser level when zoomed out. `CellCompositeIndex`, the Redis version hash, and the R2 object key all drop their level dimension back to the bare shape they already had at level 0, so no data migrated. `backfill-cell-composite-pyramid.ts` is deleted (it only ever backfilled levels 1-3). `PROTOCOL_VERSION` bumped to 10 (`CellComposite`/`SCellComposite` drop `level`).
Why: the 10 000-piece seed-lock-scenario visual verification (see above) was inconclusive at `MIN_ZOOM` (0.15): at that zoom the client picked the coarsest pyramid level, and 1% lock density spread over nearly the whole board diluted into near-invisibility after two rounds of 2x downsampling, even though the underlying bakes were already confirmed pixel-correct at every level by direct R2 inspection. Collapsing to a single level removes the option to defer that visibility problem to a coarser mipmap tier, in exchange for a simpler compositor and a test result that is easy to read directly off the one tile that exists. Confirmed with the user, who chose this over re-running the verification at zoomed-in native resolution or at the real ~995 000-piece scale (both of which the multi-level pyramid did not actually need fixing for).
Revisit when: at or near full board lock, a client at minimum zoom now asks `CompositeTileLayer` to cover the *entire* board with native-resolution tiles (up to 1296 at once) instead of a handful of coarse ones; this is exactly the unbounded zoomed-out load the pyramid (and Phase 5 as a whole) existed to close. `COMPOSITE_VRAM_BUDGET_MB` (256) and `COMPOSITE_MAX_INFLIGHT` (8) will throttle fetch concurrency and evict, but eviction never touches the currently-needed set, so at that point every needed tile is still asked for, just queued. The 100 000-piece re-run (2026-07-29, see above) already shows the *server-side* half of this: the full 1296-cell grid saturates well below full board lock (10% piece density touched every cell), so the compositor is asking for the whole grid's worth of bakes long before completion, not just at the end. The client-side "cover the whole board at once" fetch/VRAM load specifically at `MIN_ZOOM` is still unverified against a real budget; watch for this at the real ~995 000-piece scale re-run.
...
Choice: the DZI-native reveal (`?dziReveal=1`, off by default; `CompositeTileLayer` untouched and still the production renderer when the flag is off) no longer masks to locked pieces' rectangular bounds. `CellCompositor.processCell` now rasterizes a silhouette mask and a seam (border-only) texture alongside the unchanged photo composite, straight from `generatePieceGeometry` and the shared `piecePathD`/`pieceMaskSvg`/`pieceBorderSvg` helpers (now shared with the offline slicer rather than duplicated) off the seed already held in-process: no piece-tile fetch needed, unlike the photo composite. Each is baked at 3 LOD tiers (`CELL_MASK_TIER_FACTORS = [1, 4, 16]` downscale, shared via `@mpp/shared` so server and client can't drift), re-rasterizing the same already-decoded SVG per tier (`sharp().clone().resize()`, no extra geometry work) rather than re-rendering from scratch. All variants share one version bump and R2 key prefix (`<version>.avif` / `-mask-<tier>.avif` / `-seam-<tier>.avif`), so the client derives every URL from the same `{cellKey, version}` pair `region_state`/`cell_composite` already carry, plus its own tier choice from the current zoom (`dziTiles.ts`'s `maskTierForZoom`, mirroring `levelForZoom`'s zoom-to-detail mapping); no `PROTOCOL_VERSION` bump needed for any of it. Client-side, `DziRevealLayer` bakes every hydrated per-cell mask sprite into one combined `RenderTexture` (mirroring `LodTileLayer`'s own bake-to-texture technique) and assigns that single Sprite as `tileContainer.mask`, rather than a `Container` holding several sprites: confirmed directly (a WebGL pixel-readback harness, not just reading Pixi's source) that only a lone Sprite gets Pixi's real per-pixel `AlphaMask`, while a `Container` of sprites silently falls back to `StencilMask` (masks by each sprite's rectangular quad, reproducing the exact flat-rectangle bug this bake exists to fix). The seam texture draws directly on top, unmasked: the server only ever strokes a border where a piece is actually locked, so it never paints outside a valid reveal region on its own. The combined-mask rebake is also throttled (`MIN_REBAKE_INTERVAL_MS = 200`, only for a generation-only change; a real ring move rebakes immediately, never throttled), and `CELL_ASSET_VRAM_BUDGET_MB`/`CELL_ASSET_MAX_INFLIGHT` were raised (64 to 256, 8 to 16) now that tiering makes that budget cover meaningfully more cells.
Why: reuses a pipeline and R2/versioning model already proven in prod instead of building a second one, and needs no wire change. Chosen over the other option considered, raw per-piece Bezier parameters sent to the client for locked pieces only: that path would hand a still-unlocked neighbor's exact required tab/socket shape in numeric form, a materially bigger anti-programmatic-solving exposure than a raster image a bot would still have to run CV against (already out of scope, see anti-programmatic-solving). The LOD tiering, throttle, and budget changes were added after the first real-scale verification pass (994,977 / 1,000,000 locked): at native resolution a single cell's mask+seam pair alone decodes to ~33.6 MB ((cellSize+2*margin)^2 * 4 bytes * 2 textures), and the min-zoom hydrate ring at real board scale spans on the order of 300+ cells, so the previous 64 MB budget fit under 2 of them. Cells thrashed (evicted almost as soon as they hydrated) instead of ever converging, which read visually as a blocky, incomplete reveal; and since `maskGeneration` bumps on every single cell hydrate or evict, the full combined-mask re-render (up to `MAX_MASK_TEXELS`^2) fired on nearly every frame for the whole thrashing stretch, which is what actually caused the reported lag.

A second real-browser pass at that same scale (994,977 / 1,000,000) found the tiering fix from above insufficient on its own, for three compounding reasons, each fixed separately:
1. `maskTierForZoom`'s crossover picked tier by "1 native px ~= 1 screen px" sharpness (needed downscale = 1/zoom): at `MIN_ZOOM` (0.15, puzzleStage.ts) that only demands ~6.7x downscale, so the formula never reached the tier array's own coarsest entry (16x, dead code) and stayed on tier 0 (native) for any zoom above 0.25, i.e. almost the whole playable zoom range, reproducing the exact thrashing bug above at ordinary overview zoom, not just at the extreme. Replaced with fixed breakpoints (dziTiles.ts) chosen from the ring-cell-count math instead of sharpness: tier 2 below 0.3 zoom, tier 1 below 1.0, tier 0 above, each set so the worst case at its own band's low-zoom edge fits `CELL_ASSET_VRAM_BUDGET_MB`. That first pass left only ~12% headroom at the native-tier edge (~225MB of a 256MB budget) against numbers that are themselves estimates, so `CELL_ASSET_VRAM_BUDGET_MB` and `DZI_VRAM_BUDGET_MB` were both doubled (256->512, 128->256) right after, for slack against the estimate being wrong rather than against a measured shortfall.
2. The combined-mask rebake's `MIN_REBAKE_INTERVAL_MS` throttle (dziRevealLayer.ts's `updateCombinedMask`) only applied when the hydrate ring was *not* stale; a ring move was deliberately left unthrottled on the theory that "the view needs fresh coverage now". `reconcile()` runs every ticker frame regardless of camera movement, so a sustained pan keeps the ring stale on essentially every frame, turning that unthrottled path into a full up-to-`MAX_MASK_TEXELS`^2 GPU render-to-texture (sourced from every hydrated cell across the keep ring, `DEHYDRATE_MARGIN_FRAC = 0.9`, ~8x the viewport area) on every single frame for the whole gesture: the actual, dominant cause of the reported pan lag, separate from the tiering gap. The throttle now applies unconditionally; the mask sprite still repositions every frame (cheap), so its coverage rect only ever trails the camera by at most 200ms.
3. The DZI background layer (the reference photo pyramid itself, independent of the mask/seam bake) had no coarse-first fallback: `reconcileDziTiles` fetches only the single level `levelForZoom` picks for the current zoom, cold, with nothing shown until those tiles individually land. Unlike this puzzle's own reference-image thumbnail (ReferencePanel.vue, OpenSeadragon), which looks instant purely because its small viewport naturally lands on a coarse level needing only a handful of tiles, the full-window canvas at the same zoom needs far more tiles at that same formula, with no placeholder in the meantime. Fixed by `pickBaseLevel` (dziTiles.ts): a fixed, small (<=64 tile) level covering the *whole* image, fetched once and kept permanently resident beneath the dynamic zoom-level tiles (`DziRevealLayer.loadBaseLayer`), so the photo is visible everywhere the mask allows from the first frame, sharpening as finer tiles stream in on top, the same coarse-to-fine model any tiled map viewer uses.

A third real-browser pass (994,977 / 1,000,000 locked) found a different, more fundamental bug, unrelated to tiering, throttling, or the base layer: `DziRevealLayer`'s combined `maskSprite` was never added to any container, only ever assigned as `tileContainer.mask`. Pixi's mask filter resolves the mask's screen position from the mask sprite's own `worldTransform` (`FilterSystem.calculateSpriteMatrix`), which for a parentless sprite is just its bare local transform, with no camera pan/zoom applied, unlike `tileContainer` itself (a real descendant of the camera-scaled world container, see puzzleStage.ts). Confirmed live: panning the board left the revealed area fixed on screen, only jumping to a (still wrong) spot on the next rebake once the pan ended, instead of tracking the content underneath. Fixed by parenting `maskSprite` under the same `deps.container` `tileContainer` lives in, with `renderable = false` so it never draws as visible content itself, only as a mask; its `worldTransform` now composes with the camera the same way `tileContainer`'s own children do. This is also why the two earlier real-scale passes above never actually resolved the reported symptom: every fix up to this one addressed real, separate problems (VRAM thrashing, pan-time rebake cost, cold background tiles), but the reveal's on-screen alignment was broken underneath all of them, at every zoom level, not only at the min-zoom overview the first two passes focused on.

A fourth real-browser pass (994,977 / 1,000,000 locked) confirmed the parenting fix above holds (no misalignment reported) and found two new, independent defects, neither caught by the local no-crash checks the third pass's own revisit note flagged as the remaining gap, since both need real R2-backed content and a real GPU to show up. (1) The DZI background layer read visibly softer than the default `CompositeTileLayer` (AVIF) path at the same zoom and spot, confirmed by the reporter comparing the same board with `?dziReveal=1` removed. Root cause: `scripts/slice-image.ts` baked the DZI pyramid as WebP at the same numeric `--quality` the AVIF piece/composite tiles use (both defaulting to 60), but WebP is a materially less efficient codec than AVIF at an equal quality number, so identical source content at identical native resolution (`source.dzi` reports 72000x72000px, exactly `1000 cols x 72px/piece`, confirmed no downsampling) still came out visibly softer through the DZI path alone. Fixed: the DZI pyramid now bakes at its own `DZI_QUALITY` (85), decoupled from `--quality`'s AVIF-tuned default, an eyeballed approximation of AVIF-60-equivalent fidelity pending a real by-eye retune. (2) A total rendering blackout (photo gone, seam borders still visible since they are a separate unmasked container) from roughly 350% zoom to `MAX_ZOOM` (5). The formulas that vary with zoom (`levelForZoom`, `maskTierForZoom`) were re-derived and ruled out: both resolve to the exact same native level/tier across the whole 223%-500% range actually exercised, no discontinuity, and direct requests against every implicated asset (a representative fully-locked cell's photo, all 3 mask tiers, all 3 seam tiers; the DZI background tile at the same world position and level) all confirmed 200 with sane byte sizes, ruling out missing R2 data. The reporter's browser console gave the actual cause: repeated `GL_INVALID_OPERATION "Texture total allocation size is too large"`, `GL_INVALID_FRAMEBUFFER_OPERATION "...Attachment has zero size"` (on `glClear`/`glDrawElements`/`glBlitFramebuffer`), and `texImage2D: width or height out of range`, ending in "too many errors, no more errors will be reported". `glRenderbufferStorageMultisample` and `glBlitFramebuffer` (MSAA resolve) in this codebase trace to exactly one place: the mask `RenderTexture.create({..., antialias: true})`, resized up to 5x/second while zooming (`MIN_REBAKE_INTERVAL_MS`). Confirmed in `pixi.js@8.18.1`'s own source (`GlRenderTargetAdaptor.mjs`'s `_resizeColor`): an antialiased render target's multisampled color renderbuffer is fully reallocated (`gl.renderbufferStorageMultisample`) on every resize, a GL entry point plain (non-MSAA) textures never touch; on the reporter's GPU/driver, that reallocation churn under real-scale asset pressure is what failed. Fixed by dropping `antialias: true`: the mask is a binary silhouette cutout, and the true piece border already renders crisp and unmasked via `seamContainer`, so losing MSAA on the mask itself costs at most a faint edge softness, not correctness.

A fifth pass (2026-08-04) hit the same WebGL failure signature again, this time on a machine rebooted between sessions, ruling out both a stale deploy (the served bundle was fetched and checked directly: the mask texture already omits `antialias: true`) and a stale tab. Auditing every `RenderTexture.create` call in the canvas code, not just the one already implicated, found one more site sharing the identical pattern: `lodTiles.ts`'s zoom-out LOD tile bake, an unrelated system (the always-on zoom-out level-of-detail cache, not the DZI reveal) untouched by the fourth pass, and failing through a different mechanism, many tiles resident at once rather than one texture resized repeatedly, so the same unaccounted MSAA memory compounds across the resident set instead of through resize churn. Fixed the same way, dropping `antialias: true`. Not yet confirmed live: this session's own browser tooling cannot render a frame while its pane is not the visible one, so the ticker-driven board build never reaches a zoomable canvas locally, and the failure only reproduces under real GPU/driver limits at real board scale regardless, the same reason every fix in this entry has only ever been confirmed by a live report.

The same fifth-pass report continued: the reporter re-tested immediately after the `lodTiles.ts` fix deployed and hit the identical signature again, this time with a screenshot pinning it to exactly 500% zoom with the photo blank and only the seam borders visible, the fourth pass's original symptom precisely, meaning `lodTiles.ts` (a real bug in its own right, kept fixed) was never what this reporter was actually hitting. Reading Pixi's own source (`AlphaMaskPipe.mjs`, `FilterSystem.mjs`) found the actual mechanism: assigning a Sprite as `container.mask` (`tileContainer.mask = sprite`) always routes through Pixi's generic Filter system, which renders the masked container to an intermediate texture sized to its on-screen bounds before compositing with the mask, regardless of the mask sprite's own texture settings. That intermediate texture's antialiasing is never read from `dziRevealLayer.ts`'s already-fixed `maskTexture`; Pixi's built-in `MaskFilter` is hardcoded to `antialias: "inherit"`, which resolves to the *root render target's* own antialias flag, i.e. `puzzleStage.ts`'s main `Application.init({antialias: true})`. At high zoom `tileContainer`'s on-screen bounds grow with it, so this inherited-antialias intermediate texture hits the identical multisampled-renderbuffer ceiling the DZI mask and LOD tile textures already hit at their own, different, allocation points, this time with no call site inside `dziRevealLayer.ts` able to touch it: the flag being inherited lives on the renderer root, not on anything this feature owns. Fixed by dropping `antialias: true` from the main `Application.init` itself. Pieces are unaffected either way (pre-baked bitmap borders, no WebGL-time smoothing to begin with); the accepted cosmetic cost is confined to a handful of small vector-drawn (`Graphics`) UI accents: peer-cursor glyphs, the snap-flash glow, the sticky-carry outline, the play-zone frame stroke. Not yet confirmed live, nor by a local visual comparison: switching the local dev server to the small test puzzle to eyeball those accents directly hit its own admin-UI friction (a native `confirm()` dialog this session's browser tooling cannot answer) and was not pursued further, so this rests on the source-level trace above, not an observed result.

A sixth pass (2026-08-04) reported a new WebGL error signature at 500% zoom after the main-canvas antialias fix above was confirmed live: `texImage2D: width or height out of range` and `GL_INVALID_FRAMEBUFFER_OPERATION "...Attachment has zero size"` on `glClear`/`glDrawElements`, ending in the same "too many errors" cutoff, but missing the `GL_INVALID_OPERATION "Texture total allocation size is too large"` line that fingerprinted every antialias-driven crash above: a different failure mode, not a regression of the fixed one. Traced by reading Pixi's own `FilterSystem.mjs`/`AlphaMaskPipe.mjs`/`MaskFilter.mjs` source directly rather than assuming another antialias site: `MaskFilter` hardcodes `clipToViewport: false`, so `FilterSystem._calculateFilterBounds`'s own clamp to the render target's viewport size (`bounds.fitBounds(...)`) never runs for a mask pass, and `_calculateFilterArea` falls through to `instruction.container.getFastGlobalBounds(true, bounds)` since the pooled `AlphaMaskEffect` Pixi builds internally for `container.mask = sprite` never sets `filterArea` (`container.filterArea` itself, confirmed by reading `effectsMixin.mjs`, writes to an unrelated effect object the mask path never consults). That measures `tileContainer`'s full recursive global bounds, which include `baseTileContainer`'s tiles: permanently resident and spanning the whole image regardless of camera position (`loadBaseLayer`). Unlike the dynamic hydrate-ring content (bounded in world space, so roughly constant on screen as zoom changes, per `viewportRing`'s viewport-relative math in `puzzleStage.ts`), the base layer's footprint is fixed in world space, so its on-screen size scales linearly with zoom like any world-space content; past a few hundred percent it exceeds GL_MAX_TEXTURE_SIZE regardless of antialiasing, so the earlier antialias fixes could only ever change *how* that oversized allocation fails, never remove the oversized request itself. Fixed with Pixi's `Container.boundsArea`: set once in `DziRevealLayer`'s constructor and kept in sync with `keepRing` on every `reconcile()` call, it entirely replaces `getFastGlobalBoundsMixin`'s recursive children walk for bounds purposes (confirmed by reading that mixin directly), pinning the mask's intermediate texture to the same viewport-derived, zoom-independent rect the combined mask sprite itself is already sized and positioned to (`updateCombinedMask`), at no visible-content cost since nothing outside `keepRing` is ever on screen to begin with. Not yet confirmed live, the same limitation every fix in this entry has run into: this session's own browser tooling still can't drive a real GPU at real board scale.

A seventh pass (2026-08-04) reported the identical error signature again, minutes after the sixth pass's `boundsArea` fix was confirmed deployed (the live bundle's content hash was checked directly against a fresh local build of the same commit, an exact match) - except this time at 53% zoom, not past 350%, plus a new symptom: unlocked scattered pieces rendering solid black. The lower zoom threshold means the sixth pass's fix did not just fail to help, it made the failure zoom-independent, a regression. Root cause, found by reading `getFastGlobalBoundsMixin.mjs`'s `_getGlobalBoundsRecursive` line by line rather than assuming `boundsArea` behaves as documented in every context: a container's `boundsArea`, when set, is written straight into the caller's bounds accumulator, but a SEPARATE code path afterward (gated on `!!this.effects.length`, true for any container with a mask) reassigns that container's OWN local accumulator to a freshly cleared, empty `Bounds` first, then runs each effect's `addBounds` against that empty accumulator, then unconditionally merges it into the outer one regardless of what the effects loop did. `tileContainer` carries the mask effect (a real `AlphaMask` instance, confirmed via `effect.constructor.name` in a live console, not assumed from reading `AlphaMaskPipe.mjs` alone as the fifth pass had), so setting `boundsArea` directly on `tileContainer` skipped the one code path (children-recursion) that would otherwise have seeded that local accumulator with real content, leaving the effects-loop-and-merge step to operate on an accumulator that was never populated - the precise mechanism differs from a simple stray union (`AlphaMask.addBounds` intersects via `addBoundsMask`, not a plain union, and `0 * Infinity` producing `NaN` partway through a matrix transform changes which branch of that intersection's comparisons ends up taken), but the outcome traces to the same root gap either way. Fixed by moving `boundsArea` off `tileContainer` entirely, onto `baseTileContainer` instead: `baseTileContainer` carries no effects of its own, so its `boundsArea` is read directly into `tileContainer`'s real accumulator once `tileContainer` is left on the normal children-recursion path (no `boundsArea` set on it at all), with none of the effects-and-empty-accumulator machinery involved for either container.
Verification limits, stated plainly rather than glossed over: synthetic reproductions of this exact interaction, run directly against the real `pixi.js@8.18.1` build in a real browser (not hand-traced), gave inconsistent results across attempts, including cases where neither the regressed fix nor the corrected one visibly changed a deliberately-oversized test scene's computed bounds at all, which does not match either the sixth pass's confirmed-live regression or the original crash's own history. Pixi's bounds caching depends on state a real per-frame render/update pass normally keeps warm, which a scripted, non-rendering test reproduces unreliably. The fix is shipped on the strength of the source-read structural argument above (confirmed facts: `AlphaMask` is the real effect class and has `addBounds`; a container's own `boundsArea` is provably read correctly when that container has no effects; `tileContainer` did not meet that condition and `baseTileContainer` does), not on a reproduced crash-to-no-crash transition. Whether it resolves the original report or only the sixth pass's regression is unconfirmed.
Revisit when: the eighth real-browser pass (the actual next data point, since this entry's own verification could not produce one) needs to check both that the original crash is gone across the full zoom range and that no new symptom (the "pieces turn black" report from this pass, presumed a downstream rendering-state artifact of the same crash rather than a separate bug, but never independently confirmed as such) persists; the DZI pyramid quality fix still needs a real re-slice (`npm run slice`) and R2 re-upload (multi-GB) before it takes effect on `synthetic-1m`, not done yet; `lodTiles.ts` holding under ordinary zoom-out/pan and whether the root canvas losing antialiasing reads as acceptable on the small UI accents it touches are still open from earlier passes. `DZI_QUALITY` (85) is an eyeballed estimate, not a measured AVIF-60 match; retune by eye once the real photo replaces the synthetic one, the same as the mosaic builder's own tile-quality entry. Whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided.
...
### 2026-07-29, image-pipeline, mosaic builder inherits the 72px/piece gigapixel ceiling

Choice: `build-mosaic.ts --piece-size` defaults to 72, `synthetic-source.ts`'s own validated safe point (5.18 GP at a 1000x1000 grid), not CLAUDE.md's stated "~80 px per piece minimum" (6.4 GP), which already segfaults this libvips build on Windows.
Why: the render pass reuses the exact same banded chunk/strip/BigTIFF assembly already proven safe at this scale; reusing its validated ceiling avoids re-deriving a new one. Fixing the underlying segfault, or verifying a higher ceiling under the Linux prod-matching Docker container, is out of scope here.
Revisit when: re-tested after a libvips/sharp upgrade or on Linux; if either lifts the ceiling, both scripts' defaults should move together.
...
Revisit when: not yet re-verified at the real ~995 000-piece scale; the leaderboard fix above was the other bottleneck blocking that same run.

### 2026-08-03, backend-realtime, bounded per-cell piece-tile fetch concurrency and bake retries

Choice: two bounds on `CellCompositor`, shipped together. (1) `processCell`'s piece-tile fetches for one cell run at most `PIECE_FETCH_CONCURRENCY` (24) at a time via a small worker-pool `mapLimit`, instead of one `Promise.all` over the whole cell (up to ~800 pieces at full lock); `fetchPieceTile` (index.ts) also retries a failed fetch up to `PIECE_TILE_FETCH_RETRIES` (2) times with a `PIECE_TILE_RETRY_DELAY_MS` (300ms) backoff. (2) `CellCompositor.drain`'s catch block requeues a failed cell (re-added to `dirty`) up to `CELL_BAKE_MAX_ATTEMPTS` (3) total attempts, tracked per key in `bakeAttempts` and cleared on success or on giving up, before logging and dropping it for good. The requeue needs no timer: it lands behind whatever else is already in the queue, which is itself a natural backoff, and costs nothing when nothing is failing.
Why: an admin resweep over the whole grid at unbounded fetch concurrency overwhelmed the CDN and this process's own outbound connections, dropping ~36% of cells to "fetch failed"/502, with no retry at any level to recover them. The original debounced-queue design's "logged and dropped, not retried" choice (see per-cell compositing above) rested on two premises that don't hold near the end of a puzzle's life: a later lock event would re-dirty the cell and self-heal it (false once a cell is at or near 100% locked, with few or no lock events left to trigger that), and a permanently-missing composite only cost a rendering optimization the client already fell back from (false since CompositeTileLayer retired the per-piece fallback entirely, see DECISIONS: CompositeTileLayer is the only rendering path). The cell-level retry is a bounded ceiling layered on top of that same self-heal, not a replacement for it.
Revisit when: verified 2026-08-03 against the real ~99.5%-locked (994 977/1 000 000) synthetic-1m prod board, in two rounds. Round 1: a direct R2 bucket listing (`rclone lsf`, not just the Redis version index or the endpoint's own pending count) found exactly 10 of 1296 cells with zero composite at all; a targeted resweep (see admin resweep accepts an explicit cell-key list, below) fixed all 10, and server logs show zero `cell-composite` warn/error lines for the run, meaning every one baked clean on its first attempt. Round 2 found that directory-level presence does not prove completeness: a cell can hold a valid photo with no mask/seam at all, which a bare listing can't distinguish from a fully-baked cell. Checking each cell's exact current version against its full expected 7-object set (photo + 3 mask tiers + 3 seam tiers) found 454 more cells silently stuck at a pre-mask/seam-tiering bake, from before this codebase ever tiered mask/seam, untouched since because nothing had re-dirtied them. A second targeted resweep of those 454 fixed 452 clean; the remaining 2 hit a genuine non-transient failure and correctly exhausted `CELL_BAKE_MAX_ATTEMPTS`, giving up with the exact failing URL in the log instead of hanging or silently corrupting state, the first real (not hypothetical) exercise of that safety net: a specific piece-tile AVIF object (`pieces/7400/740034.avif`, and separately `pieces/7562/756292.avif`) returned a persistent 503 on every GET while succeeding on HEAD, isolated to exactly that one object among its untouched neighbors. Diagnosed, not just worked around: the locally-generated source file's size matched R2's HEAD response exactly, and a byte-identical `rclone copyto` was a no-op (rclone's own hash check found nothing to transfer), ruling out a corrupted or incomplete upload; a *forced* re-upload of those same, already-matching bytes (`--ignore-times`) immediately cleared it, which only makes sense as a stuck serving-path state at the R2/Cloudflare layer for that specific object (HEAD reads metadata and was never affected; GET streams the body and was), not a data or pipeline problem. Re-verified end to end after the fix: both cells resweept clean with zero retries logged, all 1296 cells now pass the same per-object completeness check with no exceptions. Since the object's actual bytes were never wrong, and this reproduced on a puzzle that has sat untouched for weeks, there is no evidence this is upload-time-only; a spontaneous per-object serving fault, rare but not provably excluded, remains possible for any object at any time, including during the real event. What this run does establish is that the failure mode this system needs to survive is contained and diagnosable: exactly one cell affected, a named URL in the log, no cascade, no silent corruption, recoverable by re-touching the one object.
...
Why: a full-grid resweep re-bakes every cell whether it needs it or not, at roughly one cell every 15-30s for a fully-locked ~800-piece cell even with bounded fetch concurrency and per-tile retries (see bounded per-cell piece-tile fetch concurrency above), so sweeping all 1296 cells to fix a handful of genuine stragglers costs hours of redundant CDN/R2 work and background CPU load on the same box serving live players. A real ~99.5%-locked prod audit found exactly this shape of problem: only 10 of 1296 cells actually lacked a composite (confirmed against both the Redis version index and a direct R2 bucket listing, not just the endpoint's own pending count), so targeting those 10 directly finishes in the time a couple of cells take instead of the whole grid.
Revisit when: verified 2026-08-03, twice. First at the real 10 straggler cells found above: drained in under 5 minutes (~27s/cell, consistent with the full-grid rate observed separately), versus the multi-hour full-grid sweep a first attempt at this same fix had already been left running for (observed at ~1 cell/15-30s over 1296 cells before being superseded). Second at a much larger 454-cell straggler set (see bounded per-cell piece-tile fetch concurrency above): drained cleanly in ~3.5 hours in the background with no attention needed, versus an estimated 10+ hours for a full-grid sweep of the same fix. Never expected beyond that; if straggler audits become routine rather than a rare post-incident cleanup, consider exposing this from the admin page's own UI instead of curl-only.
