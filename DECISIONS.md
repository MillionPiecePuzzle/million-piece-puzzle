...
- [The Google sync links onto the guest document](#2026-08-23-auth-and-accounts-the-google-sync-links-onto-the-guest-document-instead-of-creating-a-second-account) -> a synced account is still unreachable from a second device or once the 30-day cookie ages out; gated on the v1.1 task that puts a sign-in entry outside a live session.
...
Revisit when: a synced account is not reachable from a second device or after the 30-day cookie ages out, since the only sign-in entry sits behind a session and a linked account throws `AccountNotLinked`. Tracked as its own v1.1 task.
