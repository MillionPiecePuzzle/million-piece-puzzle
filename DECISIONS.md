...
- [Frustum culling and zoom-out render-texture LOD](#2026-05-21-frontend-canvas-frustum-culling-and-zoom-out-render-texture-lod) -> Phase 2 swaps the single render texture for pipeline-aggregated tiles if it does not hold at 1M.
...
### 2026-05-21, frontend-canvas, frustum culling and zoom-out render-texture LOD

Choice: two complementary techniques keep rendering cheap. (1) Frustum culling: groups and pieces outside the viewport toggle Pixi's `culled` flag, recomputed on every camera change, resize, and group move. It is two-level: a group whose world AABB misses the viewport is culled whole; a group that intersects has each piece tested individually, so a large partially-visible cluster renders only its on-screen pieces. Bounds are analytic (canonical offset plus one margin per piece, unioned per group), never `getBounds`. (2) Zoom-out level of detail: below `LOD_ENTER_ZOOM` (0.3) the whole world is baked once into a `RenderTexture` and shown through a single sprite (a child of `world`, so the camera transforms it like the live pieces) instead of the per-piece masked sprites; above `LOD_EXIT_ZOOM` (0.35) the live pieces return, the gap being hysteresis so a zoom hovering at the threshold does not thrash the bake. The texture is sized to the play zone with its long side capped at `LOD_MAX_TEXTURE_DIM` (2048) and never upscales a small board past 1:1. The bake calls `renderer.render` with a `Matrix` that maps world space to the texture, bypassing the camera, and clears `culled` so the entire zone is captured regardless of what is on screen; the frame and backdrop stay live crisp overlays (excluded from the bake). It refreshes on a slow cadence (`LOD_REFRESH_MS`, 1500 ms) while active and immediately on a board change (snap, spectator snapshot, drop). Clusters held right now, local or remote, are tracked in a set, excluded from the bake, and drawn live on top so a piece in hand never freezes into the texture; a released cluster stays live until the next bake folds it in, so it never blinks out.
Why: culling removes the per-piece stencil and draw-call cost for off-screen pieces, which keeps the zoomed-in playing experience smooth, but does nothing when the whole board is on screen (fully zoomed out), where every piece is visible and every mask still renders. The LOD trades that per-frame cost of ~10 000 masked-sprite draws for one sprite draw plus a periodic full-world bake, so the fully-zoomed-out view stays smooth at 10 000 pieces. It is built with `RenderTexture`, the real Phase 2 approach, not a stopgap. Rendering `world` to a texture turns it into a Pixi render group, which also makes panning cheaper (the camera becomes a single group-transform update instead of propagating to every piece).
Revisit when: Phase 2 at 1M pieces, where a single 2048-capped texture is too coarse to represent the whole board (each piece would map to a fraction of a texel) and a full-world bake is too heavy per refresh. The Phase 2 frontend-canvas task moves to pipeline-aggregated tiles if the render texture does not hold at full scale.
...
Choice: the auth secrets (`AUTH_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`) are set in the Coolify service environment, never in the Dockerfile or the repo. The non-secret auth config (`AUTH_URL`, `AUTH_TRUST_HOST`, `AUTH_COOKIE_DOMAIN`, `MPP_APP_ORIGIN`, and the rate-limit tunables) stays as Dockerfile `ENV`, consistent with the rest of the server config.
Why: a deliberate exception to deployment-config-in-code. Baking OAuth credentials or the session secret into the image (built in CI, possibly cached or shared) leaks them; they belong in the orchestrator's secret store. The non-secret host/cookie config has no such risk and stays in the image so a change is one commit plus a rebuild.
Revisit when: a real secrets manager is introduced (e.g. for rotation), at which point all secrets move there together.
