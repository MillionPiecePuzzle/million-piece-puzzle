...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> a sixth pass found a *different* WebGL crash at the same 500% zoom, unrelated to antialiasing: the mask's Filter pass sized its intermediate texture off `tileContainer`'s full unclipped bounds (`MaskFilter` hardcodes `clipToViewport: false`), which include the whole-image base layer; fixed via `Container.boundsArea` clamped to `keepRing`. None of the fixes across all six passes are re-verified live yet, and whether the antialias loss on small UI accents reads as acceptable is still a genuine open question. The DZI pyramid re-slice/re-upload the quality fix needs still hasn't run, and the replace-vs-coexist call with `CompositeTileLayer` is still open.
...
Revisit when: the DZI pyramid quality fix still needs a real re-slice (`npm run slice`) and R2 re-upload (multi-GB) before it takes effect on `synthetic-1m`, not done yet; a seventh real-browser pass is needed to confirm every fix so far holds, including the `boundsArea` clamp above (no WebGL errors through the whole zoom range including past 350%, `lodTiles.ts` holding under ordinary zoom-out/pan, and whether the root canvas losing antialiasing reads as acceptable on the small UI accents it touches, the one open question genuine uncertainty remains on). `DZI_QUALITY` (85) is an eyeballed estimate, not a measured AVIF-60 match; retune by eye once the real photo replaces the synthetic one, the same as the mosaic builder's own tile-quality entry. Whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided.
