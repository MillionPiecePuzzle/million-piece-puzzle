...
- [Edge uniqueness validated per-seed](#2026-06-09-piece-generation-edge-uniqueness-validated-per-seed), [anti-programmatic-solving via permuted wire ids](#2026-06-12-shared-protocol-anti-programmatic-solving-via-permuted-wire-ids-and-anchor-offsets) -> both resolved up to the cutover itself: the production seed is validated and `earth-mosaic` is sliced and uploaded to R2 under wire-id paths (see [Earth Mosaic slice and upload](#2026-08-15-image-pipeline-earth-mosaic-full-scale-slice-and-upload)). `MPP_PUZZLE_ID` is still `synthetic-1m` in prod; only switching it (and setting the matching `MPP_GENERATION_SEED` Coolify secret) remains.
...
Revisit when: widen the edge seed to 53 bits if the grid ever grows past 1000x1000. The production seed for `earth-mosaic` is now validated (0 collisions at 1000x1000, see DECISIONS); prod still serves `synthetic-1m` pending the puzzle cutover.
...
Revisit when: `MPP_PUZZLE_ID` is still `synthetic-1m` in the Dockerfile; `earth-mosaic`'s tile set is now sliced and uploaded under wire-id paths on R2 (see DECISIONS), matched to its own `MPP_GENERATION_SEED`, but prod has not yet been switched to serve it. Image-based (CV) solving stays explicitly out of scope.
