...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> a fourth real-scale pass confirmed the parenting fix holds and found two new bugs (DZI/AVIF codec quality mismatch, an antialiased mask texture crashing WebGL past ~350% zoom), both fixed but neither re-verified live yet; the DZI pyramid re-slice/re-upload the quality fix needs hasn't run, and the replace-vs-coexist call with `CompositeTileLayer` is still open.
...
Revisit when: the DZI pyramid quality fix needs a real re-slice (`npm run slice`) and R2 re-upload (multi-GB) before it takes effect on `synthetic-1m`, not done as part of this pass; a fifth real-browser pass is needed to confirm both fixes hold (photo sharpness at parity with `CompositeTileLayer`, no WebGL errors and full coverage through the whole zoom range including past 350%) and that the antialias removal did not reintroduce visible mask-edge aliasing worth trading back. `DZI_QUALITY` (85) is an eyeballed estimate, not a measured AVIF-60 match; retune by eye once the real photo replaces the synthetic one, the same as the mosaic builder's own tile-quality entry. Whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided.
