...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> a second real-scale pass found the tier crossover, the pan-time rebake throttle, and the missing coarse base layer all needed fixing too; a third real-browser pass at real scale and the replace-vs-coexist call with `CompositeTileLayer` are both still pending.
