...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> a third real-scale pass found the combined mask sprite had no parent so it never tracked the camera, the actual cause of the incomplete-coverage symptom the first two passes' fixes never touched; a fourth pass confirming the parenting fix holds, and the replace-vs-coexist call with `CompositeTileLayer`, are both still pending.
...
Revisit when: this bake still depends on R2 write credentials the same way the photo composite does, so the local-dev gap ("no compositor configured means locked pieces do not render") applies here too; local verification of all of the above was limited to URL construction, tier-switch tracking, and no-crash checks against a tiny test puzzle, not the actual coverage/VRAM/frame-time/alignment outcome at real scale (that needs R2-backed content, only available in a real deployment). A fourth real-browser pass confirming the parenting fix holds (full viewport coverage, mask tracking the camera through a pan, at both min-zoom and closer-in zoom levels) is the next step. The tier breakpoints `[0.3, 1.0]`, `BASE_LEVEL_MAX_TILES` (64), and the raised budget/inflight numbers are all estimates from the math in this entry, not yet empirically tuned against real usage; retune if coverage or lag are still off after that pass. Whether this replaces `CompositeTileLayer` in production or the two coexist behind a flag long-term stays deliberately undecided.
