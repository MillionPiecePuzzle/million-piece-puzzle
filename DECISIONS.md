...
- [LOD tile bake distinguishes never-streamed cells from confirmed-empty ones](#2026-08-18-frontend-canvas-lod-tile-bake-distinguishes-never-streamed-cells-from-confirmed-empty-ones) -> the placeholder tint's color/alpha were picked by reading the code, not by looking at a live render; check it actually reads as "not explored" rather than as a rendering glitch the first time it is seen zoomed out over a real gap.
...
### 2026-08-18, frontend-canvas, LOD tile bake distinguishes never-streamed cells from confirmed-empty ones

Choice: `bakeTile` paints a low-alpha placeholder (`LOD_UNKNOWN_TINT_COLOR`/`LOD_UNKNOWN_TINT_ALPHA`) into a tile instead of baking it plain transparent when the cell is `coverageSeen && !knownCells.has(key)`, then still marks the tile ready and caches it exactly as any other tile. `cellContentPending`/`computeLoadingCells` are deliberately left untouched: an earlier version of this fix also made the LOD-active branch of `cellContentPending` treat a not-yet-known cell as pending, which sounded like the same signal reused, but it fed `tickInitialFill`'s `tilesPending` gate a case `viewportStreamSettled()` does not protect against. That function returns true for a global-subscriber viewport via a branch that ignores `knownCells` entirely (nothing will ever stream there, so nothing to wait for by design), so the new `tilesPending` signal was the only thing left gating the cover, and it never clears for a cell that never streams: the exact same board this fix targets would hang behind the loading cover forever instead of showing the (now merely cosmetic) hole. Caught in review before it reached anyone; reverted, keeping only the `bakeTile` change, which does not read `cellContentPending` at all.
Why: a cell with zero local groups is ambiguous, genuinely empty or simply never streamed (see viewport-scoped initial state on join above: a global-subscriber viewport streams nothing), and the bake could not tell the two apart, so it baked both the same way. For a never-streamed cell that guess stayed wrong forever: nothing revisits a cell's tile until a group actually constructs there, which only happens once a narrower viewport scopes over it. Reported as a "hole" in the zoomed-out board that only a manual zoom-in over the exact spot would fix. Painting into the existing tile texture (rather than a separate overlay layer, closer to the removed per-cell badge) reuses the same cache/invalidation path (`markDirtyCell`/`markBaked`) with no new container or lifecycle, and fires only for this narrower, rarer case, not the ordinary hydration wait that made the old badge fire on every pan and get pulled.
Revisit when: never expected for the mechanism; the tint's color/alpha are eyeballed against the default cream backdrop with no live render seen yet, retune by eye if it reads as too visible or too subtle. If a per-cell loading signal while zoomed out is ever wanted for real (not just this placeholder), it needs its own gate, not a shared read with the initial-fill cover.
