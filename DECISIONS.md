...
Choice: `/play` exposes two buttons (Reset puzzle, Complete) wired to `dev_reset` and `dev_complete` WebSocket messages. `dev_reset` wipes and re-initializes the current puzzle; `dev_complete` jumps the locked count to the total and flips the meta status to `completed`, so the puzzle stays frozen until the next reset. Visibility is controlled by `VITE_DEV_BUTTONS` (default visible during the alpha). Server-side both messages are rejected with `dev_disabled` unless `MPP_DEV_ENABLED=1`. Both buttons are protected by a `confirm()` prompt because they affect every connected tester at once.
...
### 2026-05-28, backend-realtime, scatter decorrelated and ovaled around the frame

Choice: the initial scatter samples each piece body (origin + canonicalOffset) in an elliptical ring around the frame, then sets the group origin to body - canonicalOffset. The inner bound is the frame grown by half a piece (so a body centered beyond it never overlaps the frame interior); the outer bound is an ellipse sharing the frame aspect, scaled by `SCATTER_HALO_SCALE` (2.6, >= sqrt(2) so it encloses the inner rectangle). Each placement draws an angle and an area-uniform radius (two RNG draws) so the cloud is evenly dense and oval, matching the landscape frame.
Why: pieces render at `origin + canonicalOffset`, and `canonicalOffset` is the solved cell (`col*pieceSize, row*pieceSize`). Randomizing the origin alone left the solved image in place plus bounded jitter: top-row (sky) pieces drifted up and bottom-row (ground) pieces down, a visible gradient. Randomizing the body instead decouples the shuffled layout from the solved one. An oval halo reads better than a rectangular ring around the landscape frame, and the wider scale spaces the pieces out.
Revisit when: the halo is too dense or too sparse at a given piece count (tune `SCATTER_HALO_SCALE`).
