...
### 2026-07-27, qa-and-load, seed-lock-scenario needs a level-0 backfill companion

Choice: added `backfill-cell-composite-level0.ts`, run after `seed-lock-scenario.ts` and before `backfill-cell-composite-pyramid.ts`. It marks every level-0 cell in the frame dirty against the real compositor (same `pieceFileByWireId`/`isLocked`/`fetchTile` wiring `index.ts` uses), relying on `processLevelZeroCell`'s own empty-cell skip rather than pre-filtering to cells that actually have a locked piece.
Why: `seed-lock-scenario.ts` locks pieces by writing directly to Redis, bypassing the live dirty-cell queue a real lock event goes through, so a freshly-seeded board has zero level-0 composites. Before Stage 5 that was harmless (the per-piece fallback still rendered locked pieces); since Stage 5 removed that fallback, a directly-seeded board renders nothing locked until level 0 is baked by some other means. `backfill-cell-composite-pyramid.ts` cannot cover this itself: it only ever builds levels 1-3 from an existing level-0 bake and throws if asked to bake level 0.
Revisit when: never expected; only matters for re-seeding a synthetic lock-test scenario, not for a board locked through normal gameplay.
