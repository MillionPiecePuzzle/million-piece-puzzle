...
### 2026-05-28, backend-realtime, scatter decorrelated and ringed around the frame

Choice: the initial scatter samples each piece body (origin + canonicalOffset) uniformly from a ring around the frame, excluding the frame interior, then sets the group origin to body - canonicalOffset. The ring is the band between the frame and the frame scaled by `SCATTER_RING_SCALE` (2) about its center, split into four area-weighted bands so a placement costs three RNG draws and never lands inside the frame.
Why: pieces render at `origin + canonicalOffset`, and `canonicalOffset` is the solved cell (`col*pieceSize, row*pieceSize`). Randomizing the origin alone left the solved image in place plus bounded jitter: top-row (sky) pieces drifted up and bottom-row (ground) pieces down, a visible gradient. Randomizing the body instead decouples the shuffled layout from the solved one. Excluding the frame interior keeps the assembly area visually clear.
Revisit when: the ring is too dense or too sparse at a given piece count (tune `SCATTER_RING_SCALE`), or a non-rectangular scatter is wanted.
