...
- [Cell composites share the locked-piece residency and eviction pool](#2026-07-24-frontend-canvas-cell-composites-share-the-locked-piece-residency-and-eviction-pool) -> the 1-unit-per-composite budget approximation and the unthrottled sparse-cell fetch are both unmeasured against real usage.
...
Choice: `PROTOCOL_VERSION` (currently 8) is asserted at handshake. The client sends it in `hello`; the server closes with `protocol_mismatch` on any skew instead of letting a stale client hit malformed-message errors.
...
Choice: each rebake uploads to `<puzzleId>/cells/<cellKey>/<version>.avif` in the existing public `mpp-assets` bucket (a new key per version, never overwritten); `SRegionState.cellComposites` and the new `cell_composite` broadcast carry `{cellKey, version}`, and the frontend derives the fetch URL from those two numbers, needing no manifest indirection (a cell position is not a solved-adjacency secret). Reads (each locked piece's own tile, to composite from) go over the same plain public HTTPS the frontend already uses; only the upload needs credentials, added as `MPP_R2_ENDPOINT`/`MPP_R2_ACCESS_KEY_ID`/`MPP_R2_SECRET_ACCESS_KEY` (new `@aws-sdk/client-s3` dependency, R2's S3-compatible API). All three unset (local dev's default) leaves `ctx.cellCompositor` unconstructed and compositing fully inert rather than failing the boot.
Why: an immutable, version-suffixed key matches the caching model every other R2 asset here already has (upload once, cache forever), needing no new Cloudflare cache rule or invalidation; every other R2 interaction before this was either a boot-time manifest read or the offline `rclone`-based pipeline upload, never a write from the live process.
Revisit when: never expected for the versioning scheme. The Coolify service env needs the three secrets set before this does anything in prod (manual follow-up, see ROADMAP); orphaned R2 objects from superseded versions or a board reset are never cleaned up, accepted since R2 storage is cheap and cell composites are a small fraction of total asset size.

### 2026-07-24, frontend-canvas, composited cells replace per-piece rendering only once the tile is actually hydrated

Choice: a locked piece skips its own node only once its cell's composite sprite is both hydrated (a texture actually resident) and, via a snapshot of `lockedPieceGrid`'s cell membership taken at that hydrate, known to include that specific piece id; never merely because the cell has some composite version on record. The wire `CellComposite.cellKey` (the server's own 24-bit packing, `worldGrid.ts`) is translated into the frontend's own `CellKey` (`packCell`) via a duplicated unpack formula (`unpackWireCellKey`, `groupGrid.ts`), since the two sides intentionally use different bit layouts and only the underlying `(cx, cy)` pair is meant to cross the wire.
Why: gating on "a version exists" alone would flash-hide a piece that locks into a cell whose composite predates it, until the next debounced rebake catches up; a per-hydrate snapshot means a piece only ever stops rendering individually once something is actually already drawing it, so the fallback to Stage 2's per-piece path has no gap.
Revisit when: never expected for the correctness rule; the wire/frontend key duplication would need updating together if either side's bit layout ever changes.

### 2026-07-24, frontend-canvas, cell composites share the locked-piece residency and eviction pool

Choice: a composite sprite hydrates and dehydrates on the same viewport hydrate/keep rings as a locked piece, hides once its cell's LOD tile bakes over it, and competes in the existing `RESIDENT_PIECE_BUDGET`/LRU eviction pool alongside groups and locked pieces, counted as 1 resident unit like everything else there rather than weighted by its true (much larger) byte size. There is no dedicated spatial index or hydrate queue/cap for it: `cellComposites` is a plain map walked directly every frame, since it holds at most a few hundred entries (one per world-grid cell, not per piece) even at 1M scale.
Why: mirrors the locked-piece machinery, the closest architectural analog (per-cell, gapless LOD fill, ring-gated), instead of a second VRAM-budgeted tile cache next to it; the 1-unit approximation is safe because cell count, not budget pressure, is what actually bounds how many composites are ever resident at once. The server bakes a cell's composite on its very first lock regardless of how sparse the cell still is, so a lightly-locked cell can fetch a full `cellSize^2` tile to show only a handful of pieces; taken as the literal reading of the ROADMAP bullet (prefer the composite whenever available) rather than adding an unspecified density threshold.
Revisit when: real usage shows either the per-unit budget approximation admitting more VRAM than intended, or the sparse-cell fetch cost dominating bandwidth or memory; add a density gate (or a byte-weighted budget) then, not preemptively.
