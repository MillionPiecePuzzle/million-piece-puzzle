...
Choice: a `Swarm`, one shared instance per run, holds a single discovered `{x, y}` hotspot. Every bot, clustering or not, reports the centroid of any groups its `region_state` stream turns up; the first report logs a `[swarm] hotspot found near (...)` line, later ones just keep the hotspot fresh as pieces get dragged away. With `--cluster-fraction` (default 0) above 0, that fraction of bots center their next viewport pick on the hotspot plus roughly one viewport-width of jitter instead of a fully random point; the rest keep the original independent scatter. Two more changes ride along: a bot's drag target now samples from its own current viewport (expanded a little, floored so it's never degenerate) instead of the whole play zone, and its cursor is a small glide/track state machine (eases toward the next grab target, snaps to the live drag position while holding, goes silent once converged so the receiving client's own idle-bob still kicks in) instead of a teleport to a random point every tick.
Why: a bot can't be handed a fixed watch region because it has no legitimate way to know where pieces are ahead of time; the scatter layout derives from `generationSeed`, which is server-only and never sent over the wire (see anti-programmatic-solving above). Looking around and reporting what's found is the same discovery a real player does, just fed into one shared spot instead of one player's memory. Splitting the fraction keeps a run's broad-coverage soak characteristics intact for the bots not clustering, and viewport-local drag targets double as more realistic on their own (nobody drags a piece the width of a 1M-piece board in one motion).
Revisit when: never expected structurally. The jitter width, the cursor's 25%-per-100ms-tick ease fraction, and the drag-range expand/floor constants are eyeballed, not yet checked against a real watched run; retune by eye after the first real `--cluster-fraction` session.
