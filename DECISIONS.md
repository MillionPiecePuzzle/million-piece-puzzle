...
- [DZI reveal mask and seam baked server-side via CellCompositor](#2026-07-28-frontend-canvas-dzi-reveal-mask-and-seam-baked-server-side-via-cellcompositor) -> now baked at 3 LOD tiers after real-scale verification found the untiered version thrashed its VRAM budget; a real-browser pass at real scale and the replace-vs-coexist call with `CompositeTileLayer` are both still pending.
