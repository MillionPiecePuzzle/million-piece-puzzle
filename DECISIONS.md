...
Why: this is the same libvips write-ceiling bug as the entry above, Docker is required for any real-scale image-pipeline run on this machine, not just a one-off for this mosaic. `slice-image.ts`'s own still-unverified full-scale run (see ROADMAP) will need the same treatment: its whole-puzzle-area DZI extract is a single vips image object over the full source dimensions, exactly the pattern that breaks on Windows past ~5 GP.
