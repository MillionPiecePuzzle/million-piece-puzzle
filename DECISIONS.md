...
- [Bounded per-cell piece-tile fetch concurrency and bake retries](#2026-08-03-backend-realtime-bounded-per-cell-piece-tile-fetch-concurrency-and-bake-retries) -> verified clean against the real ~99.5%-locked synthetic-1m prod board (10/10 straggler cells fixed, zero retries needed), but `CELL_BAKE_MAX_ATTEMPTS`'s own requeue path is still untested against a real persistent (non-transient) failure, as opposed to the transient overload it was written for; watch for it at the real photo's own launch scale.
...
Revisit when: verified 2026-08-03 against the real ~99.5%-locked (994 977/1 000 000) synthetic-1m prod board: a direct R2 bucket listing (`rclone lsf`, not just the Redis version index or the endpoint's own pending count) found exactly 10 of 1296 cells actually missing, matching the Redis index exactly; a targeted resweep of just those 10 (see admin resweep accepts an explicit cell-key list, below) left both at 1296/1296, confirmed live by fetching each of a sample's photo/mask/seam AVIF URLs (200, not 404). Server logs for the whole run show zero `cell-composite` warn/error lines, meaning every one of the 10 baked clean on its first attempt: the concurrency cap and per-tile fetch retry alone were enough to fix this specific gap, so `CELL_BAKE_MAX_ATTEMPTS`'s own requeue path was not actually exercised here. It stays in place as a safety net against a whole-cell failure surviving both of those (an R2 upload error, a sharp exception, or a fetch failure run of bad luck deeper than the per-tile retry's own 2 attempts), still untested against a real persistent (non-transient) failure; if one shows up, the give-up log line names the cell so it can be resweept by hand.
...
Revisit when: verified 2026-08-03: the targeted resweep of the real 10 straggler cells found above drained in under 5 minutes (~27s/cell, consistent with the full-grid rate observed separately), versus the multi-hour full-grid sweep a first attempt at this same fix had already been left running for (observed at ~1 cell/15-30s over 1296 cells before being superseded); confirmed by re-listing R2 (1296/1296) and fetching a sample of the newly-baked AVIF URLs live. Never expected beyond that; if straggler audits become routine rather than a rare post-incident cleanup, consider exposing this from the admin page's own UI instead of curl-only.
