...
- [seed-lock-scenario locks pieces through the real grab/drop path](#2026-07-27-qa-and-load-seed-lock-scenario-locks-pieces-through-the-real-grabdrop-path) -> not yet run at 995 000-piece scale, and the wrong-content/position symptom that prompted it is not yet confirmed fixed.
...
Revisit when: not yet run at real ~995 000-piece scale (verify throughput and the per-cell fetch fan-out before assuming it scales the same way the direct-write version did); the symptom that prompted this change (a rendered locked piece showing wrong content or position, not just missing composite coverage) has not yet been reproduced against a board seeded this way, so it is not yet confirmed whether that symptom was caused by the old approach's gaps or is a separate, still-unfound bug.
