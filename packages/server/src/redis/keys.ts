...
/**
 * Hash: groupId, rotation, locked. A locked piece has no live group (see
 * DECISIONS: locked pieces stop being a group): groupId goes stale once
 * locked and is never read again, since every reader checks locked first.
 */
export const piece = (puzzleId: string, pieceId: number) => `puzzle:${puzzleId}:piece:${pieceId}`;

/** Hash: worldX, worldY, size, heldBy. Never written locked; a cluster's group is deleted, not locked, on anchor. */
