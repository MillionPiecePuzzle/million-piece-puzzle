...
// Compact panel: the leaders, plus the local user and the contributor right
// above them when the local user ranks outside the visible leaders, so the gap
// on screen is the one they can actually close. Ranked just past the leaders
// that neighbour is already on screen, and the slice clamps rather than
// repeating them. Outside the standings list entirely (nothing to slice), their
// own row comes from the personal standing the server sends them instead, so a
// contributor ranked 4000th still watches their own count move, with no
// neighbour the client holds to put above it.
const TOP_ROWS = 6;
const panelRows = computed(() => {
  ...
  if (youIndex >= TOP_ROWS)
    return [...top, ...rows.slice(Math.max(TOP_ROWS, youIndex - 1), youIndex + 1)];
