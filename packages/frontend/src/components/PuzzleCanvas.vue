...
import { SHARE_VIEW_PARAM, parseSharedView, sharedViewWorldPoint } from "../data/viewLink";
...
// The spot a hand-written framing points at. Read here rather than at the moment
// it is applied: the address bar is rewritten at boot (the analytics and auth
// flags are stripped through a router.replace) and the board is only built
// seconds later, past the guest onboarding a first-time visitor goes through.
...
async function buildStage(s: Extract<PuzzleSessionState, { kind: "ready" }>): Promise<void> {
  ...
  // A framing named in the address is applied once the board is up, since
  // build() ends on fitView and would otherwise overwrite it, and while the
  // loading cover is still down, so the player never sees the whole board first.
  // Consumed on the first build alone: a rebuild (a dev reset, a new board) must
  // not pull them back.
  if (sharedView) {
