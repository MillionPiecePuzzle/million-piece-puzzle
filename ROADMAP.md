...
- [~] Consolidate the LOD/baking/streaming reconcile: a single per-frame `reconcile()` is the sole authority for cull, LOD visibility, residency, dirty-flush and loading cells; event handlers only mutate the model and record dirty rects. Removes the scattered per-handler dirty/visibility/residency calls and the per-frame glide re-bake churn. See DECISIONS: frustum culling and tiled zoom-out LOD
