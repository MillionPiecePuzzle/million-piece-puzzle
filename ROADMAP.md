...
- [ ] A player gets back into a synced account from a second device or after the session cookie ages out: today the only sign-in entry lives in `/play`'s options menu, which already requires a session, so a returning visitor is minted a fresh guest instead, and signing in from there throws Auth.js's `AccountNotLinked` against the account linked elsewhere. Exit criterion: a synced player signs in on a device that has never seen their session and lands back on their own pseudo, country and contributions, with the guest that device just minted either folded in or abandoned deliberately.
