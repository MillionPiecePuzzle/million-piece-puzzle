...
- [ ] `shared-protocol` / `backend-realtime`: `CellComposite`/`SCellComposite` gain a `level` field, `PROTOCOL_VERSION` bump; `region_state` reports every level 0-3 tile covering its batch's cells (deduped ancestors), and the live push fires for every level, not just 0.
- [ ] `frontend-canvas`: `LodTileLayer` picks the pyramid level matching the current zoom (mipmap-style selection against `LOD_TILE_DENSITY`'s existing texel budget) and displays that level's covering tiles as fetched `Sprite`s instead of baking the live Pixi scene into a tile for locked content, reusing the tile layer's existing LRU/budget eviction. `hydrateLockedPiece`/`lockedHydrateQueue`/`lockedResident` and the per-piece fallback they exist for (`puzzleStage.ts`'s `bakeTile`, gated on `isLockedPieceCoveredByComposite`) are removed rather than kept as a second path: the only bridge between a piece anchoring and its cell's composite catching up is reusing the texture already resident from the local client's own drag (zero fetch, the same salvage already in place for a piece it just dragged itself), swapped for the composite sprite once that covers the piece. Own session: real per-level size/timing numbers from Stage 4, plus the frontend's own `CellKey` packing (`groupGrid.ts`), should inform the level-keying design before it's implemented.
