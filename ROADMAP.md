...
- [x] Viewport-driven texture streaming: per-piece AVIF textures and nodes are hydrated on demand for groups within a viewport ring and freed when they leave the keep ring or are covered by a ready LOD tile, so entering `/play` fetches only the visible window (verified at the alpha's 10 000: a fit fetched ~4 000 of 10 000 groups, panning away freed them all, returning re-queued them), and the zoomed-out view bakes its tiles from the bounded window then frees those textures. Verified at the alpha's 10 000; 1M verification shares the `Chunked board build` gate (the one-time O(board) group-container and index population still runs at `build()`)
