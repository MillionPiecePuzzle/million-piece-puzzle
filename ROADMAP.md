...
- [~] Site localized in EN/FR/ES/DE (vue-i18n): every user-facing string and both legal pages render in the four languages, a flag dropdown on the landing (top-right, square flags, custom UK/US split for English) switches the whole site, browser language auto-detected and the choice persisted to localStorage. Done: i18n foundation + landing + countdown. Pending: in-app UI (auth/pseudo/nationality modals, TopBar, canvas panels, PlayPage, toasts) and the two legal pages
