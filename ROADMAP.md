...
- [~] Rendering stays smooth on commodity hardware at 1M pieces. Landed: the world dropped `sortableChildren` for fixed z-order layer containers, so neither the first render nor any later frame sorts the whole board; the per-piece edge-geometry cache is evicted on dehydrate (see [z-order layers](DECISIONS.md#2026-06-04-frontend-canvas-z-order-layers-replace-world-sortablechildren)); and resident VRAM at a deep zoom-out is bounded by freeing covered idle clusters: the spectator snapshot now dirties only the tiles of clusters it actually changes, and a non-held cluster whose tiles all go cold is freed so only the hot working set, the bake-in-progress and held clusters stay resident (see [hot-tile residency](DECISIONS.md#2026-06-05-frontend-canvas-hot-tile-residency-frees-covered-cold-clusters)). Open: on-canvas verification at the alpha's 10 000 that the resident set drops toward the hot set at MIN_ZOOM and recovers on zoom-in; cap pieces per LOD tile (backlog "Cap maximum piece on tiles"); and full 1M runtime verification
