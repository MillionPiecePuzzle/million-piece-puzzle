...
- [x] Fix the grab/disconnect hold-leak race, add a stale-hold sweep: `client.held` reserves a grabbed group id synchronously at dispatch (not after the Redis round trip), so a disconnect racing an in-flight grab always releases it; a periodic sweep (`holds.ts`, `MPP_STALE_HOLD_MS`) force-releases any hold whose owner is gone for any other reason (crash, restart). Exit: unit tests cover the race directly (`handlers.test.ts`, `holds.test.ts`); `checkNoHeld` (`stateInvariants.ts`) now has a live runtime counterpart, not just an offline validator. See DECISIONS
