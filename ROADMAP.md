...
- [x] Site localized in EN/FR/ES/DE (vue-i18n): every user-facing string and both legal pages render in the four languages, a flag dropdown on the landing (top-right, square flags, custom UK/US split for English) switches the whole site, browser language auto-detected and the choice persisted to localStorage. Covers the i18n foundation, landing, all in-app UI (auth/pseudo/nationality modals, TopBar, canvas panels, PlayPage, loading/completion, toasts; country names via Intl.DisplayNames) and the privacy and legal-notice pages (link-bearing paragraphs via i18n-t component interpolation, last-updated date formatted per locale)
...
- [x] Admin ops page: direct-URL, Basic-auth `GET /admin` (mounted only when `MPP_ADMIN_PASSWORD` is set) to wipe all Redis+Mongo, set the event start, and switch the active puzzle. Puzzle switch and full wipe persist a Redis override and restart the container (`restart: unless-stopped`); event start applies live. Manual follow-up: set `MPP_ADMIN_PASSWORD` and `MPP_ADMIN_PUZZLES` (JSON `[{id,label,seed}]`) in the Coolify service env, then redeploy. See DECISIONS: admin ops page
