...
- [x] `frontend-canvas`: Bug. `PuzzleStage.clearWorld` detaches children with `removeChildren()` without destroying them, leaking every piece `Container` / `Sprite` / `Graphics`, the frame and the backdrop on each puzzle cycle. Exit: cleared display objects are destroyed (geometry and masks freed); no memory growth across cycles.
