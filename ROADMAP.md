...
- [~] Rendering stays smooth on commodity hardware at 1M pieces. Landed: the world dropped `sortableChildren` for fixed z-order layer containers, so neither the first render nor any later frame sorts the whole board, and the per-piece edge-geometry cache is evicted on dehydrate (see [z-order layers](DECISIONS.md#2026-06-04-frontend-canvas-z-order-layers-replace-world-sortablechildren)). Open: bound resident VRAM at a deep zoom-out window (free or degrade per-piece textures hidden behind baked tiles), and cap pieces per LOD tile
