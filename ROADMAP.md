...
- **DZI-native reveal for locked content.** Prototyped behind `?dziReveal=1` (frontend-canvas): the reference DZI pyramid as an independent zoom-driven tile layer, masked to locked pieces' bounds, no server baking. Verified fast and pixel-precise for photo content up to ~100k locked pieces; blocked on the border/seam geometry source before it can replace `CompositeTileLayer` in production. See [DECISIONS](DECISIONS.md#2026-07-28-frontend-canvas-dzi-native-reveal-prototyped-for-locked-content).
