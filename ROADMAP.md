...
- **Persisted player display settings.** Prerequisite for the reference underlay, the HUD toggles, and any locked-piece rendering option below. No player *display* preference is persisted client-side today (`localStorage` carries the guest claim token, the locale, the landing interested flag and the personal flags, none of them a display setting) and `OptionsModal.vue` holds account actions and the tips strip only, so a settings store and a display section in the menu both have to exist first.
