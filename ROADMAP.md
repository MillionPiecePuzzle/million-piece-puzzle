...
Delivered in 5 stages, each independently shippable and separately testable, since this touches the merge/anchor path (permanent, no undo) and the wire protocol. Stage 1 is backend-only and already closes the crash risk; Stage 2 makes the ~995 000-locked scenario actually testable end to end; Stage 3 is the "see it all at once" rendering polish, but only at native zoom: a zoomed-out view still falls back to per-piece rendering wherever no single-resolution composite covers it. Stage 4 replaces that single resolution with a multi-level pyramid (server storage only, no wire change yet) so a zoomed-out view has a tile at every level instead of a cliff; Stage 5 exposes it over the wire and wires the frontend onto it, retiring the per-piece fallback entirely. Each stage is meant to be picked up in its own fresh session.
...
### Stage 4 (backend): locked-region composite pyramid, levels 0-3

- [x] `backend-realtime`: generalize the debounced dirty-cell queue and the version index from a bare cell key to a `(level, key)` pair; level 0's own bake (piece-tile compositing) is unchanged. `CellCompositor.markDirty(level, cellKeys)` is called with level 0 only, from the existing lock-event and force-complete call sites; a level L>=1 cell is never marked dirty by an origin event, only by its own child's `onComposited` completion once that child's bake has actually landed (wired in `index.ts`, alongside the existing live broadcast), so a parent can never recomposite from a stale child. Levels stop at 3 (1296+324+81+25 = 1726 cells, 430 new): level 3 still covers only 1/25 of the board, so no level is ever global enough to need a broadcast tier different from level 0's own viewport scoping. See DECISIONS.
- [x] `backend-realtime`: a level L>=1 bake composites up to 4 already-baked level L-1 children (read back over the same public HTTPS as a piece tile) at their 2x2 offsets, using each child's own full, uncropped canvas rather than an analytically-cropped one, then resizes to the pyramid's one fixed per-tile pixel size (shared with level 0); the resulting fixed-pixel edge overlap (mirroring `slice-image.ts`'s reference-image Deep Zoom pyramid, `overlap: 1`) survives the resize where a world-unit margin scaled up per level would not. Uploaded under `<puzzleId>/cells/L<level>/<cellKey>/<version>.avif` for level>=1; level 0's existing bare key is untouched, so already-baked prod objects stay valid with no migration. See DECISIONS.
- [x] `backend-realtime`: this stage is server-storage only, no wire or frontend change. `region_state`/`cell_composite` keep sending exactly today's level-0-only payload (mechanically updated for the index's new per-level lookup); the frontend has no notion of `level` yet and the wire's cell-key packing is identical at every level, so a level>=1 entry would be misread as an oddly-sized level-0 one. Exposing levels 1-3 over the wire is Stage 5's job, done together with teaching the frontend to key by level. See DECISIONS.
- [x] `backend-realtime`: one-off `backfill-cell-composite-pyramid` script seeds levels 1-3 for an already-composited board (the live `synthetic-1m` board) from its current level-0 versions; a board composited from here on builds its pyramid incrementally with no backfill needed.

### Stage 5: expose the pyramid over the wire, retire per-piece locked hydration

- [x] `shared-protocol` / `backend-realtime`: `CellComposite`/`SCellComposite` gain a `level` field, `PROTOCOL_VERSION` bump to 9; `region_state` reports every level 0-3 tile covering its batch's cells (deduped ancestors, `collectRegionCellComposites`), and the live push fires for every level, not just 0, scoped by that level's own (larger) world extent.
- [x] `frontend-canvas`: a new `CompositeTileLayer` (sibling to `LodTileLayer`, not a merge into it: baking a live scene and fetching a server AVIF are different enough lifecycles) picks the pyramid level matching the current zoom (mipmap-style `desiredLevel`, hysteresis banded) and displays that level's covering tiles as fetched `Sprite`s, added directly to the existing `lockedPiecesLayer`; a needed-but-not-yet-ready level falls back to its already-hydrated finer children (`neededCompositeTiles`) so a coarse level catching up never opens a gap. `hydrateLockedPiece`/`lockedHydrateQueue`/`lockedResident` and the per-piece fallback they existed for are removed rather than kept as a second path: a locked piece never gets its own fetch again. The only bridge between a piece anchoring and its cell's composite catching up is reusing the texture already resident from the local client's own drag (`salvageLockedPiece`, zero fetch), freed once `CompositeTileLayer` confirms coverage or the piece leaves the keep ring, exempt from budget eviction (no re-fetch exists to recover it if freed early). The composite pool's own eviction is byte-weighted (real decoded size, ~17.6 MB/tile at `synthetic-1m`'s dimensions), replacing the pre-Stage-5 "1 unit like a single piece" approximation, unsafe once composites became the only rendering path. See DECISIONS.
