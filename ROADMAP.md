...
### `infra-deploy`
- [~] Production hardening: backups (Redis snapshot, Mongo dump), secrets management, DDoS posture verified. Backups: a `backup` sidecar (`docker/backup/`) pushes a gzipped `mongodump` archive plus a Redis RDB (pulled over the wire) to the private `mpp-backups` R2 bucket every 6h, pruned to the newest 3; rclone R2 creds come from the Coolify env (private bucket, since dumps carry user data). Secrets: stay in the Coolify service env, not the image (the existing posture, kept). DDoS: app-layer limits already in place (WS boundary, spectator and auth per-IP windows); `ws.*` flipped to Cloudflare-proxied with an Origin CA cert (Full strict) and a 30s server WS heartbeat added so the proxy's ~100s idle cut does not drop contributors. Pending manual infra before `[x]`: create the private bucket + R2 token and set the rclone/bucket env in Coolify; install the Origin CA cert and flip `ws.*` to orange; verify a restore; firewall the origin to Cloudflare ranges (deferred, the remaining DDoS gap)
