...
- [x] Every HUD panel is switched on and off on its own from the Display section of the options menu (reference, zoom, activity, leaderboard, minimap, flags), the choice kept per browser like the rest of the display preferences. A panel nobody has touched follows its viewport default, so a phone still opens on the board with the reference thumbnail and the minimap alone while the four it drops now have a way back onto it. Switching one off leaves every other one on the same pixel: a panel whose slot is what places a visible neighbour, the reference and the ticker on either side of the zoom pillar, stays in the column and only stops being drawn. Verified live across all eight combinations of the three. The menu itself holds a 375x812 phone with no scroll (six switches in two columns, tighter spacing below the breakpoint), measured at 721px against a 792px cap with the guest sync row in place. EN/FR/ES/DE. See DECISIONS
