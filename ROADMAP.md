...
- [x] A phone gets the board instead of panels it can only read: the leaderboard and the live activity panels are not rendered below the 680px breakpoint, leaving the reference thumbnail, the zoom controls and the minimap, each holding its own corner, and both panels come back on a widening viewport with no reload. See DECISIONS
...
- **Toggleable HUD windows.** Each panel individually hidable, driven by mobile where even the two panels left on screen compete with the board. This is the mobile navigation the responsive pass already named as its own revisit condition (see [DECISIONS](DECISIONS.md#2026-08-06-frontend-shell-the-hud-drops-its-two-reading-panels-on-a-phone-and-shrinks-the-rest)): panels collapse into a real control surface, and the leaderboard and activity ones get a way back onto a phone at all. On a phone an always-visible icon bar beats a setting buried in a modal, and that bar competes for the same screen edge as the flag bar, which already sits bottom-center on desktop.
