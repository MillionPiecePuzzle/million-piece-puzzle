...
Frontend on `http://localhost:5173`, WebSocket server on `ws://localhost:8080/`, spectator stream on `http://localhost:8080/keyframe` plus `http://localhost:8080/events/<t0>`, and the anonymous landing data on `http://localhost:8080/landing` plus `http://localhost:8080/interested`. See [CLAUDE.md](CLAUDE.md) for the working conventions, [ROADMAP.md](ROADMAP.md) for what is in flight, and [DECISIONS.md](DECISIONS.md) for the non-obvious trade-offs.
