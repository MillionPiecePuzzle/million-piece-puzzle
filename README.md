...
Contributor sign-in needs Google OAuth credentials. Set these secrets in your environment before `docker compose up` (they are never committed): `AUTH_SECRET` (a 32+ character random string, e.g. `openssl rand -hex 32`), `AUTH_GOOGLE_ID`, and `AUTH_GOOGLE_SECRET` from a Google OAuth client whose authorized redirect URI is `http://localhost:8080/auth/callback/google`. Without them the server still boots and spectator mode works; only contribution (the authenticated WebSocket) is unavailable. The non-secret auth config (`AUTH_URL`, `AUTH_COOKIE_DOMAIN`, `MPP_APP_ORIGIN`) has local defaults.
